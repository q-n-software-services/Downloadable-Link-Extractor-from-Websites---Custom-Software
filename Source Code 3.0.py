...
def regex_scraper(base_url, this_page):
    ...
    try:
        ...
        for link in newList:
            handle_link(link)
            # re_file.write(f'{link}\n')
    except:
        print("Error in Regex Scraper :\t", base_url)
...
def my_func(inputLink):
    ...
    while len(webpages) > 0 or len(anonymous_links) > 0:
        ...
        if 'https://mega.nz' in my_url:
            extracted_links.append(my_url)
            downloadable_links.append(my_url)
            done_links.append(my_url)
            continue
        ...
        if page.headers['Content-Type'] in required_types:
            extracted_links.append(my_url)
            downloadable_links.append(my_url)
        else:
